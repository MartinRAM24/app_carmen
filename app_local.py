...
# ---------- DB helpers ----------
def conn():
    return sqlite3.connect("pacientes.db", check_same_thread=False)

def exec_sql(q, p=()):
    c = conn(); cur = c.cursor(); cur.execute(q, p); c.commit(); c.close()

def df_sql(q, p=()):
    c = conn(); df = pd.read_sql_query(q, c, params=p); c.close(); return df
...
def add_col_if_missing(table: str, col: str, coldef: str):
    cols = df_sql(f"PRAGMA table_info({table})")
    if col not in cols["name"].tolist():
...
ADMIN_USER = "Carmen"
# Para demo: password en claro; en producción, usa st.secrets["ADMIN_PASSWORD"]
ADMIN_PASSWORD_HASH = sha256("admin123")  # cámbialo
...
if role == "admin":
    ...
    # Botón para nuevo paciente (modal)
    ...
    with c2:
        if st.button("🔗 Copiar link del portal del paciente"):
            ...
            # Muestra URL “local”: ajusta hostname/puerto según tu entorno
    ...
    # --- Mediciones (crear/editar + historial) ---
    with tab_medidas:
        st.caption("Registra o actualiza medidas por fecha (cada fecha es una cita).")

        # Crear/actualizar una medición por fecha (no toca los PDFs)
        ...
        if guardar_med:
            # Garantiza fila por fecha sin tocar PDFs
        ...
        # Seleccionar una medición existente para editar
    ...
    with tab_fotos:
        ...
        if gal.empty:
            ...
            for fch in sorted(gal["fecha"].unique(), reverse=True):
                    ...
                    with cols[idx % 4]:
                        ...
                        with c2:
                            if st.button("🗑️ Eliminar", key=f"del_{r['id']}"):
                                # guardamos la intención y abrimos el modal de confirmación
                ...
                # si alguien presionó eliminar, mostramos modal una sola vez
                if "_delete_photo_id" in st.session_state:
                    ...
                    break  # evita que se abran múltiples modales en el mismo render


# ---------- PACIENTE VIEW (solo lectura) ----------
elif role == "paciente":
